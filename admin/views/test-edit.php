		...
		<form method="post" id="rs-general-form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
			...
			<table class="form-table">
				<tr>
					...
					<td><input type="text" id="title" name="title" class="regular-text rs-input-wide" required dir="<?php echo esc_attr( $field_dir ); ?>"
				...
				<tr>
					<th><?php esc_html_e( 'Save & Resume', 'ravanix' ); ?></th>
					<td>
						<label>
							<input type="checkbox" name="enable_save_resume" value="1" <?php checked( $test ? $test->enable_save_resume : 0, 1 ); ?>>
							<?php esc_html_e( 'Show a "Save my progress" button on this test', 'ravanix' ); ?>
						</label>
						<p class="description"><?php esc_html_e( 'Recommended for long questionnaires. Every participant\'s answers are always autosaved in their own browser regardless of this setting; enabling it additionally shows a visible save button, and lets a logged-in participant resume from a different device by saving their progress to their account.', 'ravanix' ); ?></p>
					</td>
				</tr>
				<tr>
					<th><?php esc_html_e( 'Informed consent', 'ravanix' ); ?></th>
					<td>
						<?php $consent_mode = $test ? $test->consent_mode : 'default'; ?>
						<label style="display:block;margin-bottom:6px;">
							<input type="radio" name="consent_mode" value="default" <?php checked( $consent_mode, 'default' ); ?>>
							<?php esc_html_e( 'Use the site-wide default consent notice (set in Ravanix Settings)', 'ravanix' ); ?>
						</label>
						<label style="display:block;margin-bottom:6px;">
							<input type="radio" name="consent_mode" value="custom" <?php checked( $consent_mode, 'custom' ); ?>>
							<?php esc_html_e( 'Use a custom consent notice for this test only:', 'ravanix' ); ?>
						</label>
						<?php
						wp_editor(
							$test ? $test->consent_text : '',
							'ravanix_test_consent_text',
							array(
						...
						?>
						<p class="description"><?php esc_html_e( 'Only used when "custom" is selected above.', 'ravanix' ); ?></p>
						<label style="display:block;margin-top:6px;">
							<input type="radio" name="consent_mode" value="disabled" <?php checked( $consent_mode, 'disabled' ); ?>>
							<?php esc_html_e( 'Do not show a consent step for this test', 'ravanix' ); ?>
						</label>
					</td>
				</tr>
				<tr>
