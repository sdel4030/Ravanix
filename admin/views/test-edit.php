...
if ( $test_id && ! $test ) {
	echo '<div class="wrap"><p>' . esc_html__( 'The requested test was not found.', 'ravanix-lite' ) . '</p></div>';
	return;
}
...
<div class="wrap rs-wrap" dir="<?php echo is_rtl() ? 'rtl' : 'ltr'; ?>">
	<h1><?php echo $is_new ? esc_html__( 'Add New Test', 'ravanix-lite' ) : esc_html__( 'Editing test:', 'ravanix-lite' ) . ' ' . esc_html( $test->title ); ?></h1>

	<?php if ( $is_new ) : ?>
		<p class="description"><?php esc_html_e( 'First save the test\'s general information; you can then add dimensions, questions, and interpretations.', 'ravanix-lite' ); ?></p>
	<?php else : ?>
		<?php
		/**
		...
		$tabs = apply_filters(
			...
			array(
				array( 'slug' => 'general', 'label' => __( 'General Info', 'ravanix-lite' ) ),
				array( 'slug' => 'dimensions', 'label' => __( 'Dimensions / Subscales', 'ravanix-lite' ) ),
				array( 'slug' => 'questions', 'label' => __( 'Questions', 'ravanix-lite' ) ),
				array( 'slug' => 'interpretations', 'label' => __( 'Interpretation Ranges', 'ravanix-lite' ) ),
				array( 'slug' => 'preview', 'label' => __( 'Preview and shortcode', 'ravanix-lite' ) ),
			),
			$test_id
		);
		?>
	...
	<?php if ( isset( $_GET['saved'] ) ) : ?>
		<div class="notice notice-success is-dismissible"><p><?php esc_html_e( 'Changes saved successfully.', 'ravanix-lite' ); ?></p></div>
	<?php endif; ?>
	<?php if ( isset( $_GET['assigned'] ) ) : ?>
		<div class="notice notice-success is-dismissible"><p><?php
			/* translators: %d: the number of questions that were just assigned to a dimension. */
			printf( esc_html__( '%d question(s) assigned to this dimension.', 'ravanix-lite' ), intval( $_GET['assigned'] ) );
		?></p></div>
	<?php endif; ?>
	<?php if ( $is_new || 'general' === $tab ) : ?>
		...
		<form method="post" id="rs-general-form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
			...
			<table class="form-table">
				<tr>
					<th><label for="title"><?php esc_html_e( 'Test title', 'ravanix-lite' ); ?></label></th>
				...
				<tr>
					<th><label for="slug"><?php esc_html_e( 'Slug — optional', 'ravanix-lite' ); ?></label></th>
					<td>
						<input type="text" id="slug" name="slug" class="regular-text" dir="ltr"
							value="<?php echo $test ? esc_attr( $test->slug ) : ''; ?>" placeholder="<?php esc_attr_e( 'e.g. beck-depression-inventory', 'ravanix-lite' ); ?>">
						<p class="description"><?php esc_html_e( 'For better site SEO. If left empty, a slug is generated automatically. Only English letters, numbers, and hyphens are allowed.', 'ravanix-lite' ); ?></p>
					</td>
				</tr>
				<tr>
					<th><label for="ravanix_description"><?php esc_html_e( 'Short description', 'ravanix-lite' ); ?></label></th>
					<td>
						<?php
						wp_editor(
						...
						?>
						<p class="description"><?php esc_html_e( 'This text is shown to the user before starting the test, with formatting (bold, lists, paragraphs, etc.).', 'ravanix-lite' ); ?></p>
					</td>
				</tr>
				<tr>
					<th><label for="instructions"><?php esc_html_e( 'Test instructions', 'ravanix-lite' ); ?></label></th>
					<td><textarea id="instructions" name="instructions" rows="4" class="large-text" dir="<?php echo esc_attr( $field_dir ); ?>" placeholder="<?php esc_attr_e( 'Text shown to the user before starting the test...', 'ravanix-lite' ); ?>"><?php echo $test ? esc_textarea( $test->instructions ) : ''; ?></textarea></td>
				</tr>
				<tr>
					<th><label for="tags"><?php esc_html_e( 'Tags', 'ravanix-lite' ); ?></label></th>
					<td>
						<input type="text" id="tags" name="tags" class="regular-text"
							value="<?php echo $test ? esc_attr( $test->tags ) : ''; ?>" placeholder="<?php esc_attr_e( 'e.g. depression, screening, clinical', 'ravanix-lite' ); ?>">
						<p class="description"><?php esc_html_e( 'Separate with commas. Used to display "Related questionnaires" on the custom post type page.', 'ravanix-lite' ); ?></p>
					</td>
				</tr>
				<tr>
					<th><label for="categories"><?php esc_html_e( 'Category', 'ravanix-lite' ); ?></label></th>
					<td>
						<?php
						...
						<?php if ( ! empty( $existing_categories ) ) : ?>
							<p class="description"><?php esc_html_e( 'Choose from the categories already on this site:', 'ravanix-lite' ); ?></p>
						...
						<input type="text" id="categories" name="categories" class="regular-text"
							value="<?php echo esc_attr( implode( ', ', $custom_selected ) ); ?>" placeholder="<?php esc_attr_e( 'Or type a new category, e.g.: Personality tests', 'ravanix-lite' ); ?>">
						<p class="description"><?php esc_html_e( 'For a new category, separate names with commas. Categories that don\'t already exist will be created automatically. These values are shown on the archive and single post type pages (if enabled).', 'ravanix-lite' ); ?></p>
					</td>
				</tr>
				<tr>
					<th><label for="featured_image_id"><?php esc_html_e( 'Featured image', 'ravanix-lite' ); ?></label></th>
					<td>
						...
						<button type="button" class="button" id="rs-select-featured-image"><?php esc_html_e( 'Select Image', 'ravanix-lite' ); ?></button>
						<button type="button" class="button rs-link-danger" id="rs-remove-featured-image" style="<?php echo ( $test && $test->featured_image_id ) ? '' : 'display:none;'; ?>"><?php esc_html_e( 'Remove image', 'ravanix-lite' ); ?></button>
						<p class="description"><?php esc_html_e( 'This image is shown to users on the questionnaire archive page and at the top of this test\'s single page (if the custom post type is enabled).', 'ravanix-lite' ); ?></p>
					</td>
				</tr>
				<tr>
					<th><?php esc_html_e( 'Publish status', 'ravanix-lite' ); ?></th>
					<td>
						<select name="status">
							<option value="draft" <?php selected( $test ? $test->status : 'draft', 'draft' ); ?>><?php esc_html_e( 'Draft', 'ravanix-lite' ); ?></option>
							<option value="published" <?php selected( $test ? $test->status : '', 'published' ); ?>><?php esc_html_e( 'Published (visible on the site)', 'ravanix-lite' ); ?></option>
						</select>
					</td>
				</tr>
				<tr>
					<th><label for="questions_per_page"><?php esc_html_e( 'Questions per page', 'ravanix-lite' ); ?></label></th>
					<td>
						<input type="number" id="questions_per_page" name="questions_per_page" min="1" style="width:100px;"
							value="<?php echo ( $test && $test->questions_per_page ) ? intval( $test->questions_per_page ) : ''; ?>" placeholder="<?php esc_attr_e( 'All on one page', 'ravanix-lite' ); ?>">
						<p class="description"><?php esc_html_e( 'For long questionnaires (such as the NEO-PI-R with 240 questions), specify how many questions to show per page. If left empty, all questions are shown on a single page (the default behavior).', 'ravanix-lite' ); ?></p>
					</td>
				</tr>
				<tr>
					<th><?php esc_html_e( 'User login', 'ravanix-lite' ); ?></th>
					<td>
						<label>
							<input type="checkbox" name="require_login" value="1" <?php checked( $test ? $test->require_login : 1, 1 ); ?>>
							<?php esc_html_e( 'Only logged-in users can take this test', 'ravanix-lite' ); ?>
						</label>
					</td>
				</tr>
				<tr>
					<th><?php esc_html_e( 'Result display order', 'ravanix-lite' ); ?></th>
					<td>
						<label>
							<input type="checkbox" name="rank_results" value="1" <?php checked( $test ? $test->rank_results : 0, 1 ); ?>>
							<?php esc_html_e( 'Show results ranked from highest to lowest', 'ravanix-lite' ); ?>
						</label>
						<p class="description"><?php esc_html_e( 'Useful for questionnaires where the relative ranking between dimensions matters (e.g. top strengths, career interests, dominant personality traits), rather than each dimension\'s absolute clinical level. When enabled, dimension and composite-factor scores are sorted by score on the result page and PDF instead of the admin-defined order.', 'ravanix-lite' ); ?></p>
					</td>
				</tr>
				<?php
				/**
				...
				<tr>
					<th><?php esc_html_e( 'Questionnaire URL on this site', 'ravanix-lite' ); ?></th>
					<td>
						<a href="<?php echo esc_url( get_permalink( $test->cpt_post_id ) ); ?>" target="_blank"><?php echo esc_html( get_permalink( $test->cpt_post_id ) ); ?></a>
						—
						<a href="<?php echo esc_url( get_edit_post_link( $test->cpt_post_id ) ); ?>"><?php esc_html_e( 'Edit the linked post', 'ravanix-lite' ); ?></a>
					</td>
				</tr>
				<?php endif; ?>
			</table>

			<?php submit_button( $is_new ? __( 'Save and continue', 'ravanix-lite' ) : __( 'Save Changes', 'ravanix-lite' ) ); ?>
		</form>
